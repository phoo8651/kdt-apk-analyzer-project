...
/* 앱 내부 files 디렉터리 경로 (※ 실행은 onJavaReady 내부에서만 호출됨) */
...
console.log("[+] AllInOne Recon Script Loaded");

/* =========================
 *         메인 훅
 * ========================= */
onJavaReady(function () {
  ...
  try {
    ...
    Editor.putString.implementation = function (key, value) {
      jlog({ type: "pref", sub: "putString", key: String(key), value: String(value) });
      return this.putString(key, value);
    };
  } catch (e) { /* 클래스가 없을 수 있음 */ }

  /* ---------- 파일 I/O (FileOutputStream.write) ---------- */
  try {
    const Arrays = Java.use('java.util.Arrays');
    const FOS = Java.use('java.io.FileOutputStream');
    const fosWrite = FOS.write.overload('[B', 'int', 'int');
    fosWrite.implementation = function (buffer, offset, count) {
      const slice = Arrays.copyOfRange(buffer, offset, offset + count);
      let preview = bytesToString(slice);
      if (preview) preview = preview.replace(/[^\x20-\x7E]/g, '.').substring(0, 100);
      jlog({ type: "fileio", sub: "write", count: count, preview: preview || "" });
      return fosWrite.call(this, buffer, offset, count);
    };
  } catch (e) { /* Ignore */ }
  ...
  try {
    ...
    const enc2str = Base64.encodeToString.overload('[B', 'int');
    enc2str.implementation = function (input, flags) {
      const result = enc2str.call(this, input, flags);
      const preview = (bytesToString(input) || "").substring(0, 100);
      jlog({ type: "encoding", sub: "base64", preview: preview });
      return result;
    };
  } catch (e) { /* Ignore */ }

  /* ---------- 네트워크 (OkHttp3 / HttpURLConnection) ---------- */
  try {
    const RealCall = Java.use("okhttp3.RealCall");
    if (RealCall.execute) {
      const exec = RealCall.execute;
      RealCall.execute.implementation = function () {
        const req = this.request();
        jlog({ type: "net", sub: "okhttp3.execute", method: String(req.method()), url: String(req.url().toString()) });
        return exec.call(this);
      };
    }
    if (RealCall.enqueue) {
      const enq = RealCall.enqueue;
      RealCall.enqueue.implementation = function (callback) {
        const req = this.request();
        jlog({ type: "net", sub: "okhttp3.enqueue", method: String(req.method()), url: String(req.url().toString()) });
        return enq.call(this, callback);
      };
    }
  } catch (e) { /* OkHttp3 미사용 가능 */ }

  try {
    const HttpURLConnection = Java.use("java.net.HttpURLConnection");
    const getOS = HttpURLConnection.getOutputStream;
    HttpURLConnection.getOutputStream.implementation = function () {
      const url = this.getURL().toString();
      jlog({ type: "net", sub: "HttpURLConnection", method: String(this.getRequestMethod()), url: url });
      return getOS.call(this);
    };
  } catch (e) { /* Ignore */ }
  ...
  try {
    ...
    loadUrl.implementation = function (url) {
      jlog({ type: "webview", sub: "loadUrl", url: String(url) });
  ...
  try {
    ...
    Method.invoke.implementation = function (obj, args) {
      ...
      jlog({ type: "reflection", sub: "Method.invoke", method: String(name) });
  ...
  try {
    ...
    function dumpToFile(bin, label) {
      try {
        if (!bin) return null;
        const Arrays = Java.use('java.util.Arrays');
        const cap = Math.min(bin.length, 65536); // 64KiB cap
        ...
        if (arguments.length >= 3) {
          ...
          if (p && p.$className) {
            ...
            try {
              ...
              else if (ptype === "javax.crypto.spec.GCMParameterSpec") { iv = toHex(p.getIV()); tlen = p.getTLen(); }
            } catch (_) { }
          }
        }
    ...
    if (Cipher.doFinal && Cipher.doFinal.overload && Cipher.doFinal.overload('[B', 'int', 'int')) {
      const df3 = Cipher.doFinal.overload('[B', 'int', 'int');
      df3.implementation = function (b, o, l) {
        const rec = Meta.get(this) || { id: -1, opm: 0, alg: this.getAlgorithm() };
        const phase = opmodeToStr(rec.opm);
        const Arrays = Java.use('java.util.Arrays');
        const in_slice = (b && l > 0) ? Arrays.copyOfRange(b, o, o + l) : null;
        const in_path = dumpToFile(in_slice, "crypto-in-" + phase);
        const ret = df3.call(this, b, o, l);
        const out_path = dumpToFile(ret, "crypto-out-" + phase);
    ...
    // (선택) update
    if (state.hookUpdate && Cipher.update && Cipher.update.overload && Cipher.update.overload('[B')) {
      try {
        const upd = Cipher.update.overload('[B');
        upd.implementation = function (b) {
          const rec = Meta.get(this) || { id: -1, opm: 0, alg: this.getAlgorithm() };
          const phase = opmodeToStr(rec.opm);
          const in_path = dumpToFile(b, "crypto-upd-in-" + phase);
          const ret = upd.call(this, b);
  ...
  // 추가: DexClassLoader (동적코드 로딩 가시화)
  try {
    const DCL = Java.use('dalvik.system.DexClassLoader');
    DCL.$init.implementation = function (dexPath, optimizedDirectory, librarySearchPath, parent) {
      jlog({ type: "runtime", sub: "DexClassLoader", dex: String(dexPath), odex: String(optimizedDirectory), lib: String(librarySearchPath) });
      return this.$init(dexPath, optimizedDirectory, librarySearchPath, parent);
    };
  } catch (e) { jlog({ type: "warn", sub: "DexClassLoader hook failed", error: String(e) }); }
...
rpc.exports = {
  ...
  profile: function (name) {
    if (name === "scan") { state.mode = "auto"; state.back = "block"; state.finish = "block"; state.uiforce = true; }
    else if (name === "free") { state.mode = "free"; state.back = "free"; state.finish = "free"; state.uiforce = true; }
